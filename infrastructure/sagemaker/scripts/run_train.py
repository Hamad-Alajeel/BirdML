...
def main() -> None:
    ...
    if args.skip_training.lower() == "true":
        ...
        logger.info("Loading dataset to regenerate class_names.json...")
        ...
        logger.info("Skipping training and MLflow sync.")
        return
    ...
    if params is None:
        logger.warning("No best_params.json found at s3://%s/%s", S3_BUCKET, S3_BEST_PARAMS_KEY)
        logger.warning("Falling back to BEST_PARAMS from config.")
        params = BEST_PARAMS
    else:
    ...
    logger.info("=== Step 3/4: Running two-stage training ===")
    checkpoint_path = run_training(dataset=dataset, params=params)

    logger.info("=== Step 4/4: Uploading artifacts to S3 ===")
